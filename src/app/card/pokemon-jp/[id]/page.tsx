    ...
    return (
      <div className="min-h-screen bg-[var(--bg)] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-400 mt-4 text-sm">Loading card details...</p>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="min-h-screen bg-[var(--bg)] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-300 font-medium">Card not found</p>
          <button onClick={() => router.back()} className="mt-4 px-6 py-2 bg-[var(--surface-1)] text-gray-300 rounded-xl hover:text-amber-400 transition-colors">
            Go Back
          </button>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-[var(--bg)]">
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        {/* Back button */}
        <button onClick={() => router.back()} className="flex items-center gap-2 text-gray-400 hover:text-amber-400 transition-colors mb-6 text-sm">
          ← Back to search
        </button>
        ...
        <div className="grid md:grid-cols-[320px_1fr] gap-8">
          ...
          <div className="bg-[var(--card-bg)] border border-[var(--card-border)] rounded-2xl p-4 flex items-center justify-center">
            {card.imageUrl ? (
              ...
              <div className="w-full aspect-[2.5/3.5] bg-[var(--surface-1)] rounded-xl flex items-center justify-center text-gray-500">
                No Image
              </div>
            )}
          </div>
          ...
          <div className="space-y-6">
            ...
            <div>
              <div className="flex items-center gap-3 mb-2">
                <span className="px-3 py-1 bg-yellow-500/15 text-yellow-400 rounded-lg text-xs font-bold uppercase tracking-wider">
                  Pokemon JP
                </span>
                {card.rarity && (
                  <span className="px-3 py-1 bg-amber-500/15 text-amber-400 rounded-lg text-xs font-bold">
                    {card.rarity}
                  </span>
                )}
                {card.variant && (
                  <span className="px-3 py-1 bg-purple-500/15 text-purple-400 rounded-lg text-xs font-medium">
                    {card.variant}
                  </span>
                )}
              </div>
            ...
            {hasPrices && (
              <div className="bg-[var(--card-bg)] border border-[var(--card-border)] rounded-2xl overflow-hidden">
                <div className="px-5 py-4 border-b border-[var(--card-border)]">
                  ...
                  <p className="text-xs text-gray-500 mt-0.5">TCGplayer market prices by condition</p>
                </div>
                ...
                {card.lastPriceUpdate && (
                  <div className="px-5 py-3 border-t border-[var(--card-border)] text-xs text-gray-500">
            ...
            {card.tcgplayerId && (
              <a
                ...
                className="inline-flex items-center gap-2 px-5 py-2.5 bg-blue-600 hover:bg-blue-500 text-white rounded-xl transition-colors font-medium text-sm"
              >
                View on TCGplayer →
              </a>
            )}
