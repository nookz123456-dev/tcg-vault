      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {cards.length === 0 ? (
          <div className="text-center py-16">
            ...
            <Link href="/search" className="inline-block px-6 py-3 bg-[#6366f1] text-white font-bold rounded-xl hover:bg-[#4f46e5] transition-all shadow-sm shadow-[#6366f1]/25">
              Search Cards →
            </Link>
          </div>
        ) : (
          <div className="space-y-3">
            {cards.map((card) => (
              <div key={card.id} className="bg-white border border-[#e8eaf0] rounded-xl p-4 flex items-center gap-4 card-hover shadow-sm">
                ...
                <div className="text-center">
                  ...
                  <div className="flex items-center gap-1">
                    <button onClick={() => card.quantity > 1 && updateCard(card.id, { quantity: card.quantity - 1 })} className="w-6 h-6 bg-[#f5f6fa] rounded text-[#8b8fa6] hover:text-[#1e2235] text-xs border border-[#e8eaf0]">-</button>
                    <span className="text-sm text-[#1e2235] font-bold w-6 text-center">{card.quantity}</span>
                    <button onClick={() => updateCard(card.id, { quantity: card.quantity + 1 })} className="w-6 h-6 bg-[#f5f6fa] rounded text-[#8b8fa6] hover:text-[#1e2235] text-xs border border-[#e8eaf0]">+</button>
                  </div>
                </div>
